...
class ScatterAndGatherAsync(Controller):
    def __init__(
        self,
        ...
        search_space_id=AutoFedRLConstants.SEARCH_SPACE_ID,
    ...
    def start_controller(self, fl_ctx: FLContext) -> None:
        ...
        if not isinstance(self.search_space, FLComponent):
            self.system_panic(
                f"Search_space {self.search_space_id} must be a FLComponent type object, "
    ...
    def control_flow(self, abort_signal: Signal, fl_ctx: FLContext) -> None:
        try:
            ...
            # Define a queue to store the names of clients that have completed their tasks
            client_name = Queue()
            ...
            while self._current_round < self._start_round + self._num_rounds:
                ...
                fl_ctx.set_prop(AutoFedRLConstants.VAL_ROUND, val_num, private=True, sticky=True)
                self.fire_event(AppEventType.ROUND_STARTED, fl_ctx)

                # if update_ss:
                #     self.search_space.sample_hyperparamters(const=AutoFedRLConstants.GLOBAL_HYPERPARAMTER_COLLECTION, fl_ctx=fl_ctx)
                ...
                if self._current_round==self._start_round:
                    ...
                    hp = fl_ctx.get_prop(AutoFedRLConstants.GLOBAL_HYPERPARAMTER_COLLECTION)
                    data_shareable.set_header(AutoFedRLConstants.GLOBAL_HYPERPARAMTER_COLLECTION, fobs.dumps(hp))

                    self.aggregator.update_aggregation_weights(fl_ctx)

                    train_task = Task(
                        name=AutoFedRLConstants.TRAIN_AND_VALIDATE,
                        data=data_shareable,
                        props={},
                        timeout=self._train_timeout,
                        before_task_sent_cb=self._prepare_train_task_data,
                ...
                if self._current_round>self._start_round and self._current_round<(self._num_rounds-self._min_clients+1):
                    # if update_ss:
                    #     # Create val_task for hp search
                    #     data_shareable_val: Shareable = self.shareable_gen.learnable_to_shareable(self._global_weights, fl_ctx)
                    #     data_shareable_val.set_header(AppConstants.CURRENT_ROUND, self._current_round)
                   ...
                   # Create train_task
                    ...
                    sent_target = client_name.get()
                    self.log_info(fl_ctx,f"task(train) will be sent to {sent_target}")
                    self.send(
                        task=train_task,
                        fl_ctx=fl_ctx,
                        targets=[sent_target],
                    )
                    num_val_result = self.aggregator_search_val.get_len()
                    self.log_info(fl_ctx,f"accpet {num_val_result} search_val results")
                    if self.aggregator_search_val.get_len() == self._min_clients:
                        # aggregate val loss for hp search
                        self.fire_event(AppEventType.BEFORE_AGGREGATION, fl_ctx)
                        aggr_val_result = self.aggregator_search_val.aggregate(fl_ctx)
                        self.fire_event(AppEventType.AFTER_AGGREGATION, fl_ctx)
                ...
                while True:
                    agg_len = self.aggregator.get_len()
                    if agg_len!=0:
                        break
                    else:
                        time.sleep(5)
                self.fire_event(AppEventType.BEFORE_AGGREGATION, fl_ctx)
                agg_client, aggr_result = self.aggregator.aggregate(fl_ctx, data_shareable)
                # Store the name of the client that has completed its task into the queue
                client_name.put(agg_client)
    ...
    def _prepare_train_task_data(self, client_task: ClientTask, fl_ctx: FLContext) -> None:
        self.log_info(fl_ctx,"before task send")
