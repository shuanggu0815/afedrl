...
class PTAutoFedRLSearchSpace(FLComponent):
    ...
    def handle_event(self, event_type: str, fl_ctx: FLContext):
        if event_type == EventType.START_RUN:
            ...
            if self.search_ne:
                hyperparams_points += [self.ne_range for _ in range(self.n_clients)]
            if self.search_aw:
    ...
    def sample_hyperparamters(self,const, fl_ctx: FLContext) -> None:
        ...
        if self.search_aw:
            self.log_info(fl_ctx, f"Aggregation weights: {aw}")
        if self.search_slr:
        ...
        hps = {
            ...
            "aw": aw if self.search_aw else None,
            "slr": slr if self.search_slr else None,
        }
        if const == AutoFedRLConstants.GLOBAL_HYPERPARAMTER_COLLECTION:
            fl_ctx.set_prop(const, hps, private=True, sticky=False)
        elif const == AutoFedRLConstants.CLIENT_HYPERPARAMTER_COLLECTION:
            fl_ctx.set_prop(const, hps, private=True, sticky=False)
        else:
            self.log_error(fl_ctx, f"Could not handle Const: {const}")
    ...
    def update_search_space(self, shareable, fl_ctx: FLContext) -> None:
        ...
        if dxo.data_kind == DataKind.METRICS:
            ...
            udata = dxo.data["udata"] if "udata" in dxo.data else None
            udis = dxo.data["udis"] if "udis" in dxo.data else None
        else:
            raise ValueError("data_kind should be DataKind.METRICS, but got {}".format(dxo.data_kind))
    
        val_loss = val_loss * (udata+udis)
        self.val_losses.append(torch.tensor(val_loss, dtype=torch.float32, device=self.device))

        self.log_info(
            fl_ctx,
            f"val_loss: ({val_loss}), udata: ({udata}), udis: ({udis})")
        start = time.time()
        current_round = fl_ctx.get_prop(AutoFedRLConstants.VAL_ROUND)
        ...
        if len(val_losses_cut) > 1:
            ...
            self.log_info(fl_ctx, (f"self.logprob_history {self.logprob_history}"))
            ...
            # current_loss = self.logprob_history[-2] * (current_improvements[-1] - current_mean_improvements)
        # Update search space
        if not (type(current_loss) == int):  # TODO: Is this needed?
            self.optimizer.zero_grad()
            # if udata is not None and udis is not None:
            #     loss = (udis * udata) - current_loss
            #     # loss = -1 * current_loss/(udis * udata)
            # else:
            #     loss = -1 * current_loss
            (-1*current_loss).backward(retain_graph=True)
            self.optimizer.step()
...
class LearnableGaussianContinuousSearchDRL(torch.nn.Module):
    def __init__(self, hyperparams_points, initial_precision=None, device="cpu", rl_nettype="mlp"):
        ...
        self.mean = Parameter(torch.zeros(self.dim) + 10e-8)
    ...
    def forward(self):
        ...
        updated_mean = self.mean + mean_update
        updated_precision_component = self.precision_component + precision_component_update
        updated_mean = torch.clamp(updated_mean, -1.0, 1.0)

        dist = MultivariateNormal(
            loc=updated_mean, precision_matrix=torch.mm(updated_precision_component, updated_precision_component.t())
        )
        sample = dist.sample()
