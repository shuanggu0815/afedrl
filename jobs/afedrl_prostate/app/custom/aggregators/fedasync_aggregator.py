...
from aggregators.dxo_async_aggregator import DXOAsyncAggregator
...
class FedAsyncAggregator(Aggregator):
    ...
    def accept(self, shareable: Shareable, fl_ctx: FLContext) -> bool:
        ...
        if contributor_name in self.site_round:
            self.site_round[contributor_name] += 1
        else:
            self.site_round[contributor_name] = 0
        ...
        self.log_info(fl_ctx,f"site_round: {self.site_round}")
    ...
    def get_len(self):
        for key in self.expected_data_kind.keys():
            length = self.dxo_aggregators[key].get_len()
        return length
    
    def aggregate(self, fl_ctx: FLContext, global_weight: Shareable) -> Tuple[str, Shareable]:
        ...
        Args:
            ...
            global_weight (Shareable): The global model's weights

        Returns:
            str: The name of the client that has completed its task.
            Shareable: The aggregated and weighted shareable from contributors.
        """
        ...
        for key in self.expected_data_kind.keys():
            client_name, aggregated_dxo = self.dxo_aggregators[key].aggregate(fl_ctx,global_weight)
            if key == self._single_dxo_key:  # return single DXO with aggregation results
                return client_name, aggregated_dxo.to_shareable()
